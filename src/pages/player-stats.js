...
export default function PlayerStats() {
  ...
  useEffect(() => {
    axios
      .get(
        `${endpoint}dbInteractions/allrows?tableName=PlayerStats&serverId=${selectedServer}`,
      )
      .then(response => {
        if (
  ...
  const columns = [
    ...
    },
    {
      name: "online",
      label: "Online",
      options: { display: false, hint: "1 means online and 0 means offline" },
    },
    { name: "lastLogin", label: "Last Login" },
    {
      name: "dateCreated",
      label: "1st Recorded Join",
      options: { hint: "When the player first joined" },
    },
  ]
